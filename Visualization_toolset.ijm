...
----------------------------------------------------------------------------------------------------------------------*/
macro "Gray [n1]"		{
	if (isKeyDown("space")) toggleChannel(1);
	else run("Grays");
}
macro "Green [n2]"		{
	if (isKeyDown("space")) toggleChannel(2);
	else run("Green");
}
macro "Red [n3]"		{
	if (isKeyDown("space")) toggleChannel(3);
	else run("Red");
}
macro "Cyan [n4]"		{
	if (isKeyDown("space")) toggleChannel(4);
	else run("Cyan");
}
macro "Magenta [n5]"	{
	if (isKeyDown("space")) toggleChannel(5);
	else run("Magenta");
}
macro "Yellow [n6]"		{
	if (isKeyDown("space")) toggleChannel(6);
	else run("Yellow");
}
macro "[n7]"			{
	if (isKeyDown("space")) toggleChannel(7);
}
...
macro "Auto contrast all Ch [R]"	{run("Auto-contrast on all channels");}
...
macro "About these tools" {
	...
	"None of these tools will modify the pixels of the images.\n"+" \n"+
	...
	"      Press ctrl (pc) or cmd (mac) key to access the built-in shortcuts of Imagej\n"+" \n"+
	...
	"i.e. you can delete sortcuts, change the [key] or add more LUTs to the Set LUTs list.",13);
	Dialog.show();
}
